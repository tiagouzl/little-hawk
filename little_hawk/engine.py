"""Motor transformer: LlamaLayer (atenção + SwiGLU) e MultiLayerEngine com StreamingKVCache O(1)."""

import math

import numpy as np

from ._ansi import GREEN, RESET


class LlamaLayer:
    def __init__(
        self, W_q, W_k, W_v, W_o, rms_attn, gate, up, down, rms_ffn, n_heads, d_k, b_q=None, b_k=None, b_v=None
    ):
        self.n_heads = n_heads
        self.d_k = d_k
        self.d_model = W_q.shape[0]
    ...
    def attn_step(self, x_t, k_cache, v_cache, win_ptr, inv_freq, S, W, max_cap, wbi, si, n_ctx):
        x_n = self._rms_norm(x_t, self.rms_attn)
        B = 1
        _q = x_n @ self.W_q
        _k = x_n @ self.W_k
        _v = x_n @ self.W_v
        ...
        n_sink = min(n_ctx, S)
        n_win = max(0, min(n_ctx - S, W))
        if n_win < W:
            win_ctx = np.arange(S, S + n_win, dtype=np.int64)  # crescendo
        else:
            win_ctx = (wbi + win_ptr + 1) % W + S  # estado estacionário
        ...
        #   Q      → max_cap-1        (fixo: modelo "acha" que está sempre no fim)
        if n_ctx <= max_cap:
...
class MultiLayerEngine:
    def __init__(
        ...
        self.S = sink_size
        self.W = window_size
        self.max_cap = sink_size + window_size
        self.V = vocab_size
        self.bos_id = 1
        self.eos_id = 2
        ...
        self._init_rope(rope_base)
        self._init_idx()

    def _init_rope(self, base):
        i = np.arange(0, self.d_k, 2, dtype=np.float32)
        self.inv_freq = 1.0 / (base ** (i / self.d_k))
    ...
    def load_weights(self, path):
        ...
        self.d_model = int(data["_meta_d_model"])
        self.n_heads = int(data["_meta_n_heads"])
        self.d_k = self.d_model // self.n_heads
        self.n_layers = int(data["_meta_n_layers"])
        self.V = int(data["_meta_vocab_size"])
        self.bos_id = int(data["_meta_bos_id"])
        self.eos_id = int(data["_meta_eos_id"])
        ...
        expected = {
            "embed": (self.V, self.d_model),
            "lm_head": (self.V, self.d_model),
            "norm_w": (self.d_model,),
        }
        for i in range(self.n_layers):
            ...
            for n in ("b_q", "b_k", "b_v"):
                if f"L{i}_{n}" in data:
                    expected[f"L{i}_{n}"] = (self.d_model,)
        errs = []
        for key, shape in expected.items():
            if key not in data:
                errs.append(f"ausente: {key}")
                continue
            if shape is None:
                continue
            got = tuple(data[key].shape)
            if got != shape:
                errs.append(f"{key}: shape {got} ≠ esperado {shape}")
        inter = None
        for i in range(self.n_layers):
            g = tuple(data[f"L{i}_gate"].shape)
            d = tuple(data[f"L{i}_down"].shape)
            if g[1] != self.d_model or d[0] != self.d_model or g[0] != d[1]:
                errs.append(f"L{i}: gate{g}/down{d} incoerentes com d_model={self.d_model}")
            if inter is None:
                inter = g[0]
            elif g[0] != inter:
                errs.append(f"L{i}: intermediate {g[0]} ≠ {inter} (L0)")
        if errs:
            raise ValueError(
                f"Pesos inválidos em '{path}':\n  "
                + "\n  ".join(errs[:12])
        ...
        self._init_idx()
        print(
            f"  {GREEN}✓{RESET} {self.n_layers} camadas  d_model={self.d_model}  "
    ...
    def step(self, token_id, caches, win_ptr, n_ctx):
        ...
        for li, layer in enumerate(self.layers):
            kc, vc = caches[li]
            ao, kc, vc, sm = layer.attn_step(
                x, kc, vc, win_ptr, self.inv_freq, self.S, self.W, self.max_cap, self.wbi, self.si, n_ctx
            )
            x = x + ao
            x = x + layer.ffn(x)
