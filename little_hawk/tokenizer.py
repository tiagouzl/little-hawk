"""BPE Tokenizer (demo + modo doador) e decode incremental byte-safe."""
...
from ._ansi import CYAN, GREEN, RESET, _hdr
...
class BPETokenizer:
    ...
    def train(self, corpus, vocab_size=512, verbose=True):
        ...
        if verbose:
            print(f"\r  [{'█' * 20}] {len(self.merges)}/{nm}  {GREEN}✓{RESET} vocab={len(at)}{' ' * 20}\n")
    ...
    def encode(self, text, add_bos=True):
        ...
        unk = self.vocab.get(self.UNK, 0)
        ids = [self.vocab[self.BOS]] if add_bos else []
        for w in self._pretokenize(text):
            ids.extend(self.vocab.get(t, unk) for t in self._tok_word(w))
        return ids
    ...
    def decode(self, ids):
        ...
        for sp in [self.PAD, self.UNK, self.BOS, self.EOS]:
    ...
    def load_donor_vocab(self, meta_path):
        ...
        try:
            ...
            with open(meta_path, encoding="utf-8") as _f2:
                _meta2 = json.load(_f2)
