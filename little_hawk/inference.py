"""Loop de inferência autoregressiva com amostragem e painel de telemetria."""
...
from ._ansi import BOLD, CYAN, DIM, GREEN, MAGENTA, RED, RESET, WHITE, YELLOW
from .tokenizer import BPETokenizer, StreamDecoder
...
class LittleHawkInference:
    ...
    def _sample(self, logits, temperature=1.0, top_k=40, top_p=0.92, rep_penalty=1.3, generated=None, rng=None):
    ...
    def _panel(self, step, wp, sm, ts, lat, ev, nl):
        ...
        return [
            ...
            f"  {DIM}latência{RESET}  {WHITE}{lat:>5.1f} ms{RESET}",
            f"",
            f"  {DIM}cache [{GREEN}sink{RESET}{DIM}|{RESET}{CYAN}janela{RESET}{DIM}]{RESET}",
            f"  [{cb}]",
            f"  {DIM}{fi}/{self.max_cap} slots  ({pct:.0f}% sink){RESET}",
            f"",
            f"  {DIM}sink L0 (tok[0]){RESET}",
            f"  [{smb}] {csm}{sm:.1f}%{RESET}",
            f"",
            f"  {DIM}último token{RESET}",
            f"  {YELLOW}{ts!r:<18}{RESET}",
            f"{DIM}{'─' * 44}{RESET}",
        ]

    def generate(
        self, prompt, max_tokens=80, temperature=0.7, top_k=40, top_p=0.92, rep_penalty=1.15, stream=True, panel=True
    ):
        ...
        for step in range(max_tokens):
            ...
            nid = self._sample(
                last_logits.copy(), temperature, top_k, top_p, rep_penalty=rep_penalty, generated=generated
            )
            n_ctx += 1
            ...
            if stream:
                if panel and step > 0 and step % 8 == 0:
                    if pl:
                        sys.stdout.write(f"\033[{pl}A\033[J")
        ...
        if stream:
            print(f"\n\n{DIM}{'═' * 72}{RESET}")
            if panel:
                for ln in self._panel(len(generated), win_ptr, sm, ts, lat, ev, self.engine.n_layers):
                    print(ln)
