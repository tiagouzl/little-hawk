...
def extract(st_path, n_layers):
    ...
    available = list(sf.keys())
    print(inf(f"{len(available)} tensores no arquivo"))
    ...
    pkg = {
        ...
        "_meta_intermediate": np.float32(INTERMEDIATE),
        "_meta_n_layers": np.float32(n_layers),
        "_meta_vocab_size": np.float32(VOCAB_SIZE),
        "_meta_rope_base": np.float32(ROPE_BASE),
        "_meta_bos_id": np.float32(BOS_ID),
        "_meta_eos_id": np.float32(EOS_ID),
    }

    hdr(f"Camadas 0 … {n_layers - 1}")
    for i in range(n_layers):
        ap = f"model.layers.{i}.self_attn"
        mp = f"model.layers.{i}.mlp"
        ln1 = f"model.layers.{i}.input_layernorm"
        ln2 = f"model.layers.{i}.post_attention_layernorm"
        ...
        ap_ = (W_q.size + W_k.size + W_v.size + W_o.size) / 1e6
        mp_ = (gate.size + up.size + down.size) / 1e6
...
""")
    ...
    if args.inspect:
        ...
        for l, v in [
            ("d_model", str(D_MODEL)),
            ("intermediate", f"{INTERMEDIATE} (MLP)"),
            ("n_heads (Q)", str(N_HEADS)),
            ("n_kv_heads", f"{N_KV_HEADS} (GQA {GQA_RATIO}:1)"),
            ("d_k", str(D_K)),
        ...
        mlp = 2 * D_MODEL * INTERMEDIATE + INTERMEDIATE * D_MODEL
        print()
        for n in [1, 2, 4, 8, 12]:
            mb = (n * (attn + mlp) * 4) / 1e6
            print(inf(f"{n:>2} camadas → ~{mb:.0f} MB no .npz"))
        return

    hdr("Download")
    print(warn("Usa cache local se já baixado\n"))
    try:
        st_path = download_file("model.safetensors", args.cache_dir)
    except Exception as e:
        print(err(f"Falhou: {e}"))
        sys.exit(1)
    ...
    print(ok(f"{args.output}  ({mb:.1f} MB)"))
