...
class BPETokenizer:
    ...
    def train(self, corpus, vocab_size=512, verbose=True, progress_callback=None):
        """
        Treina o tokenizer BPE.
        - verbose: se True, chama progress_callback (se fornecido) ou não faz prints.
        - progress_callback: função opcional chamada a cada progresso relevante.
        """
        wl = self._pretokenize(corpus)
        wf = Counter()
        for w in wl:
            wf[" ".join(w)] += 1
        base = set()
        ...
        at = sp + sorted(base)
        vw = dict(wf)
        nm = max(0, vocab_size - len(at))
        self.merges = []
        if verbose and progress_callback:
            progress_callback("start", {"corpus_len": len(corpus), "n_words": len(wf), "vocab_size": vocab_size})
        for step in range(nm):
            pairs = self._count_pairs(vw)
            if not pairs:
                break
            best = pairs.most_common(1)[0][0]
            vw = self._merge_pair(best, vw)
            self.merges.append(best)
            at.append(f"{best[0]}{best[1]}")
            if verbose and progress_callback and step % 50 == 0:
                progress_callback("step", {"step": step, "nm": nm, "pair": best})
        if verbose and progress_callback:
            progress_callback("end", {"n_merges": len(self.merges), "nm": nm, "vocab_size": len(at)})
        self.vocab = {t: i for i, t in enumerate(at)}
        self.id_to_token = {i: t for t, i in self.vocab.items()}
        self.bos_id = self.vocab.get(self.BOS, 1)
        self.eos_id = self.vocab.get(self.EOS, 2)
        self._trained = True
    ...
    def load_donor_vocab(self,meta_path):
        ...
        if not self.vocab:
            raise RuntimeError(
                f"Vocabulário vazio: '{meta_path}' não embute 'vocab' e o "
                f"tokenizer.json do doador não está no cache HF. "
                f"Rode novamente o script de transplant para regenerar o _meta.json.")
...
class StreamDecoder:
    """Decodificação token a token sem quebrar UTF-8 multibyte.

    Problema: em byte-level BPE, um caractere como ’ (U+2019, 3 bytes) pode
    ser dividido entre dois tokens. Decodificar cada token isoladamente
    produz mojibake ('âĢĻ'). Solução: reconstruir os bytes crus de cada
    token via tabela reversa GPT-2 e alimentar um decoder UTF-8 incremental,
    que retém sequências parciais até ficarem completas.
    """
    def __init__(self,tok):
        self.tok=tok
        self.u2b={v:k for k,v in BPETokenizer._bytes_to_unicode().items()}
        self.dec=codecs.getincrementaldecoder("utf-8")(errors="replace")
    def push(self,tid:int)->str:
        ts=self.tok.id_to_token.get(tid)
        if not ts:return ""
        if not self.tok._donor_mode:
            # Modo demo: cada token é uma peça inteira de caractere(s)
            return ts.replace("Ġ"," ").replace("Ċ","\n").replace(BPETokenizer.SPACE," ")
        try:
            raw=bytes(self.u2b[c] for c in ts)
        except KeyError:
            # Token especial/não byte-level (<|im_end|>, etc.) → literal UTF-8
            raw=ts.encode("utf-8")
        return self.dec.decode(raw)
    def flush(self)->str:
