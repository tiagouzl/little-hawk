...
class LlamaLayer:
    ...
    def _write_cache(self, k, v, k_cache, v_cache, n_ctx, win_ptr, S, slot_override):
    ...
    def _compute_context(self, n_ctx, win_ptr, S, W, max_cap, wbi, si, ctx_override):
        ...
        return ctx, pos_ctx, pos_q

    def _attend(self, q, k_cache, v_cache, ctx, pos_ctx, pos_q, inv_freq):
        B=1
        ...
        return out, float(at[:,:,0,0].mean()*100), at
    ...
    def attn_step(self,x_t,k_cache,v_cache,win_ptr,inv_freq,S,W,max_cap,wbi,si,n_ctx,slot_override=None,ctx_override=None):
        ...
        k_cache, v_cache, _ = self._write_cache(k, v, k_cache, v_cache, n_ctx, win_ptr, S, slot_override)
        ctx, pos_ctx, pos_q = self._compute_context(n_ctx, win_ptr, S, W, max_cap, wbi, si, ctx_override)
        out, sm0, at = self._attend(q, k_cache, v_cache, ctx, pos_ctx, pos_q, inv_freq)
        return out, k_cache, v_cache, sm0, at
